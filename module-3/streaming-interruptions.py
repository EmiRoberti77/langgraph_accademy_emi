...
model = ChatOpenAI(model="gpt-4o",openai_api_key=api_key, temperature=0.4)
...
while(True):
    ...
    print(prompt)
    print("-----------")    
    if prompt == 'exit' or prompt == 'x':
        break;
    config = {"configurable":{"thread_id":"1"}}
    
    for event in graph.invoke({"messages":[HumanMessage(content=prompt)]}, config, stream_mode="values"):
